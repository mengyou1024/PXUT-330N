...
int                  main() {
    ...
    */
    ...
    if (MGetLanguage() > 1 || MGetLanguage() < 0)
        MSetLanguage(0);
    MSetUnitType(0);
...
void TestGainTable() {
    ...
    if (Input_Number(9 * C_ECHAR_HDOT, 0, &FixGain, 3, &deci_len, 0) < 1)
        return;
...
void TestGainTable1() {
    ...
    METextOut(0, 0, "Begin:", 6);
    number = 0;
    if (Input_Number(7 * C_ECHAR_HDOT, 0, &number, 2, &deci_len, 0) < 1)
        return;
    ...
    for (; i <= 120; i++)
        GainTable[i] = GainTable[i - 1] + step;
