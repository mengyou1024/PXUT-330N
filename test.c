                    ...
                    if (Input_Number(xpos, ypos, &number, 4, &deci_len, 0) != 1)
                        ...
                        if (number <= 30000) {
                            ...
                            if (MGetProbeMode() == C_P_TRANSMISSION)
                                testrange[2] = number * 3;
                            else
                                testrange[2] = number * 2;
                            break;
                        } else
                            DisplayPrompt(4);
                    }
...
int TestOffset(uint32_t testrange1, uint32_t testrange2, uint32_t mode) {
    ...
    */
    ...
    if (MGetProbeMode() == C_DPROBEZ) {
        ...
        if (offset0 > 160 * 8)
            offset0 = (offset0 - 160 * 8) / 2 + 160 * 8;
        if (offset0 > 160 * 4)
            offset0 = (offset0 - 160 * 4) / 2 + 160 * 4;
        if (offset0 > 160 * 2)
            offset0 = (offset0 - 160 * 2) / 2 + 160 * 2;
        ;
    ...
    if (MGetUnitType() > 0)
        EMenuOut(menu_xpos + 9 * C_ECHAR_HDOT + 40 * MGetLanguage(), menu_ypos + 20 + 20 * MGetLanguage(), "in", 2, 1, 8);
    else
        EMenuOut(menu_xpos + 9 * C_ECHAR_HDOT + 72 * MGetLanguage(), menu_ypos + 20 + (crow_height - 4) * 1 * MGetLanguage(), "mm", 2, 1, 8);
    ...
    while (1) {
        ...
        if (keycode == C_KEYCOD_CONFIRM && confirm_num == 0) {
            ...
            if (testrange2 <= testrange1) {
                ...
                if (MGetProbeMode() == C_DPROBEZ || MGetProbeMode() == C_DPROBEA)
                    MSetGatePara(C_COORHORIUNIT * 2 - 7, C_COORHORIUNIT * 4, 120, 0, C_SETMODE_SETSAVE);
                else
                    MSetGatePara(C_COORHORIUNIT * 4 - 7, C_COORHORIUNIT * 1.5, 120, 0, C_SETMODE_SETSAVE);
    ...
    if (confirm_num > 1) {
        ...
        if (MGetProbeMode() == C_APROBE || MGetProbeMode() == C_DPROBEA) {
            ...
            while (1) {
                if (Input_Number(menu_xpos, menu_ypos, &number, 2, &deci_len, 0) != 1)
                    break;
                else {
...
int TestMenuAngle(void) {
                    ...
                    if (Input_Number(xpos, ypos, &number, 2, &deci_len, 0) != 1)
                        ...
                        if (number <= 9000) {
                            if (number < 1000) /*斜探头输入K值*/
                                ...
                                MSetAngle(number / 10, C_SETMODE_SAVE); /*设置当前通道	*/
    ...
    if (retvalue == 1 && MGetAngle(0) > 0) {
        ...
        MSetSystem();
        while (1) {
            if (TestAngle(testrange[0], testrange[1]) == C_FALSE)
                return C_FALSE;
            return C_TRUE;

            //;if( DisplayQuery(12) == 1) return C_TRUE;
        }
    } else if (retvalue == 1 && MGetAngle(0) == 0)
        return C_TRUE;
    return C_FALSE;
}
...
int TestAngle(uint32_t diameter, uint32_t depth) {
    ...
    if (MGetUnitType() > 0)
        EMenuOut(xpos + 9 * C_ECHAR_HDOT + 40 * MGetLanguage(), ypos + 20 + 20 * MGetLanguage(), "in", 2, 1, 8);
    else
        EMenuOut(xpos + 9 * C_ECHAR_HDOT + 94 * MGetLanguage(), ypos + 22 + (crow_height - 4) * 1 * MGetLanguage(), "mm", 2, 1, 8);
...
int TestMenuDac(void) {
        ...
        */
        ...
        switch (keycode) {
            ...
            case C_KEYCOD_DEL:
                ...
                retvalue = 1;
#if C_DEVTYPE == 14
                if (C_KEYCOD_POINT == keycode)
                    retvalue = 2;
#else
                if (C_KEYCOD_DEL == keycode)
                    retvalue = 2;
#endif
                offset = C_OFF_TEMP + 30;
                MCopyProtectedMemory((void *)offset, testrange, 12, PM_COPY_WRITE);
    ...
    if (retvalue >= 1) {
        while (1) {
            ...
            if (TestDac(testrange[0], testrange[1], testrange[2], retvalue) == C_FALSE)
                return C_FALSE;
...
int TestDac(uint32_t depthmax, uint32_t diameter, uint32_t length, short test_num) {
    ...
    if (depthmax > 1500 && MGetPulseMode() == 0)
        MSetPulseMode(1, C_SETMODE_SETSAVE);
    ...
    if (test_num > 0) {
        ...
        TextOut(menu_xpos + C_ECHAR_HDOT - 10 * MGetLanguage(), menu_ypos + 4, 1, 28 + 5 * MGetLanguage(), C_CCHAR_VDOT, (uint8_t *)_TestMenu_AD2[MGetLanguage()][0], 4);
        ...
        if (keycode == C_KEYCOD_CONFIRM && test_num != 0) {
            ...
            while (xpos > 0) // 当SearchPeak返回-1时表示未能找到最高波
            {
                ...
                if (keycode == C_KEYCOD_CONFIRM) {
                    ...
                    ClearCursor(2);
                    if (test_num > 0)
                        DrawDac(1); // 参数为1表示清DAC
                    if (MGetTestStatus(C_MAKE_DACAVG) == 1)
                        test_num = 0;
                    dac_dist[test_num] = MGetRange(3) * xpos / C_COORWIDTH; // 得到采样点距离
                    if (MGetUnitType() > 0)
                        dac_dist[test_num] = dac_dist[test_num] * 254 / 1000; // 花成MM
...
int CalcuDac(void) {
    ...
    range = MGetRange(3);
    if (MGetUnitType() > 0)
        range = range * 254 / 1000;
    delay = MGetDelay(3);
    if (MGetUnitType() > 0)
        delay = delay * 254 / 1000;
    ...
    if (MGetTestStatus(C_MAKE_DACAVG) == 1 || MGetTestStatus(C_TEST_AVG) == 1) {
        ...
        if (dac_dist[0] <= MGetNearField(near_time)) {
            ...
            test_num++;
            if (MGetNearField(near_time) < dac_dist[2] / 5)
...
//-------dac-------
    ...
    */
    ...
    if (dac_line[dist2] > 32767)
        dac_line[dist2] = 32767;
    if (MGetFunctionMode(C_DEPTH_COMPENSATE) == 1) {
        dist1           = dist2;
        dist2           = C_COORWIDTH;
        dac_line[dist2] = dac_line[dist1];
...
int TestMenuFft(void) {
    ...
    if (Input_Number(xpos + (17 + MGetLanguage() * 12) * C_ECHAR_HDOT, ypos, &number, 2, &deci_len, 0)) {
        ...
        if (Input_Number(xpos + (17 + MGetLanguage() * 12) * C_ECHAR_HDOT, ypos, &number, 2, &deci_len, 0)) {
            ...
            if (MGetFrequence() < 400)
                MSetBandMode(0, C_SETMODE_SETSAVE);
            else if (MGetFrequence() < 800)
                MSetBandMode(1, C_SETMODE_SETSAVE);
            else if (MGetFrequence() < 1200)
                MSetBandMode(2, C_SETMODE_SETSAVE);
            else
                MSetBandMode(3, C_SETMODE_SETSAVE);
            ...
            if (number > 1000)
                MSetScaleDelay(number - 40, C_SETMODE_SETSAVE);
            else
                MSetScaleDelay(number * 96 / 100, C_SETMODE_SETSAVE);
...
int TestMenuSensitivity(void) {
    ...
    if (retvalue == 1) {
        while (1) {
            if (TestSensitivity(testrange[0], testrange[1], 0) == C_FALSE)
                return C_FALSE;
            if (DisplayQuery(12) == 1)
                return C_TRUE;
        }
...
int TestSensitivity(uint32_t depth, uint32_t stdamp, int mode) {
    ...
    if (MGetUnitType() > 0)
        EMenuOut(xpos + 11 * C_ECHAR_HDOT + 40 * MGetLanguage(), ypos + 12 + 20 * MGetLanguage(), "in", 2, 1, 8);
    else
        EMenuOut(xpos + 11 * C_ECHAR_HDOT + 40 * MGetLanguage(), ypos + 12 + 20 * MGetLanguage(), "mm", 2, 1, 8);
    ...
    while (1) {
        ...
        } else if (mode == 0 && keycode == C_KEYCOD_BASEGAIN && GetSameKeyTime() > 20)
            MAdjustGain(0, 0, stdamp * 2, stdamp * 2);
        else if (keycode == C_KEYCOD_GATE || keycode == C_KEYCOD_BASEGAIN || keycode == C_KEYCOD_RANGE) {
            KeyManage(keycode, 1);
            MKeyRlx();
            DisplayPrompt(15);
...
int ManualMonthTest(void) {
    ...
    uint16_t _Prompt_TB1[] =
        {
            CH(5039), CH(3887), CH(4852), CH(3660), CH(4254), CH(2215), CH(1866), CH(4252)}; //	仪器性能手动测试
...
int MenuMonthTest(void) {
    ...
    while (retvalue) {
        ...
        if (begin == 0) {
            if (CMessageBox(0, 0, (char *)_Prompt_B[MGetLanguage()][5], 24, MB_YESNO) == MB_NO)
                retvalue = true;
            else
                retvalue = false;
            begin = 1;
                ...
                if (keycode == C_KEYCOD_CONFIRM) {
                    ...
                    for (i = 0; i < 4; i++) {
                        ...
                        for (j = 0; j < 10; j++) {
                            ...
                            if (gateamp < 105 && gateamp > 95)
                                break;
                        }
                        if (i == 0)
                            power.gain[0] = MGetBaseGain();
                        MParaRenovate(1);
                        ...
                        if (number > 4)
                            number -= 4;
                        if (number > power.Horiline)
                            power.Horiline = number;
                        ExpendTime(50);
                    }
                    break;
        ...
        while (retvalue) {
            ...
            while (retvalue) {
                ...
                if (keycode == C_KEYCOD_CONFIRM) {
                    ...
                    // ExpendTime(200);
                    if (gateamp <= 1)
                        power.ResolvingPower = 400;
                    else
                        power.ResolvingPower = 200 * log10(100.0 / gateamp) + 40;
            ...
            while (retvalue) {
                ...
                if (keycode == C_KEYCOD_CONFIRM) {
                    ...
                    for (i = 0; i < 20 && MGetSysGain() > 0; i++) {
                        ...
                        if (gateamp > 0)
                            power.Dynamic = MGetBaseGain();
                        number = gateamp * 5 - table[i];
                        if (i < 13) {
                            // 计算垂直线性
                            if (number >= 0) {
                                if (number > power.vertline)
                                    power.vertline = number;
                            } else {
                                if (number < number2)
                                    number2 = number;
                            }
...
int WeldSetup(void) {
    ...
    while (1) {
        ...
        if (stWeldPara.type == 3) {
            ...
            // 钝边加坡口深大于板厚
            if (stWeldPara.thick1 < stWeldPara.deep)
                stWeldPara.deep = stWeldPara.thick1;
        ...
        if (stWeldPara.type < 3) {
            // 单面/双
            if (stWeldPara.angle1 > 1500)
                stWeldPara.angle1 = 1500;
            if (stWeldPara.angle1 < stWeldPara.angle2)
                stWeldPara.angle2 = stWeldPara.angle1 / 2;
            if (stWeldPara.angle3 > 1500)
                stWeldPara.angle3 = 1500;
            if (stWeldPara.angle3 < stWeldPara.angle4)
                stWeldPara.angle4 = stWeldPara.angle3 / 2;
        ...
        do {
            ...
            keycode = MGetKeyCode(10);
#if C_DEVTYPE == 14
            if ((keycode > 0 && keycode <= row_number) || (keycode == C_KEYCOD_PLUS || keycode == C_KEYCOD_MINUS || keycode == C_KEYCOD_RIGHT || keycode == C_KEYCOD_LEFT) || (keycode == C_KEYCOD_CONFIRM || keycode == C_KEYCOD_RETURN))
#else
            if ((keycode > 0 && keycode <= row_number) || (keycode == C_KEYCOD_PLUS || keycode == C_KEYCOD_MINUS) || (keycode == C_KEYCOD_CONFIRM || keycode == C_KEYCOD_RETURN))
#endif
            {
                MKeyRlx();
                break;
                    ...
                    while (true) {
                        if (Input_Number(xpos, ypos, &number, 2, &deci_len, 0) != 1)
                            ...
                            if (stWeldPara.type == 3) {
                                ...
                                // 钝边加坡口深大于板厚
                                if (stWeldPara.thick1 < stWeldPara.deep)
                                    stWeldPara.deep3 = stWeldPara.thick1;
                                if (stWeldPara.type < 2) {
...
int ShowWeldA(short lxpos, short lypos, short width, short height, short angle, int dist) {
    ...
    while (true) {
        if (MAnyKeyReturn() == C_KEYCOD_RETURN)
            break; // 按了返回键
    }
