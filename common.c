...
extern uint16_t       GainTable[];
extern const uint16_t c_gaintable[][121];
...
extern const uint16_t _ParaMenu_CE2[];
...
void ExpendTime(int time) /*消耗时间，以10ms为单位*/
{
    ...
    while (1) {
        ...
        if (GetElapsedTime() > PreElapsedTime)
            break;
        PreElapsedTime /= 2;
        PreElapsedTime *= 2;
        // MGetKey();
...
void BuzzerOn(uint32_t time) /*蜂鸣0.1 * time 秒，如果参数time=0则转换当前蜂鸣状态*/
{
    //	MSetAlarmStatus(1);
    //	return;
    uint32_t PreElapsedTime = GetElapsedTime() + 10 * time * 4;
    if (time >= 0) {
...
{
    uint32_t PreElapsedTime = GetElapsedTime() + 3;
    //	return;
...
uint32_t GetNoKeyTime(void) // 得到到从前次按键到目前止的时长，单位s
{
    int offset;
    int keyintime = 0;
...
uint32_t CheckDate(uint32_t year, uint32_t month, uint32_t date) // 检查年、月、日是否符合
{
    // 检查年、月、日是否符合规则，返回值0符合，非0不符合，1：year,2:month,4:date
    // 1+2:year+moth,1+4:year+date,2+4:month+date,1+2+4:year+month+date
    uint32_t ret_year = 0, ret_month = 0, ret_date = 0;
...
int test_fft(void) {
    ...
    for (i = 0; i < C_MAX_DOT_NUMBER; i++) {
        ...
        if (temp0 < C_COORWIDTH)
            dataI[i] = ((sampbuff[temp0] - 128)) * 20;
        else
            dataI[i] = 0;
    }
    FFT(dataI, w, mode);
...
uint32_t MLoadSystemElapsedTime(int mode) {
...
uint32_t MGetSystemTime(int mode) {
    ...
    uint32_t   date, year, month, day, hour, minute, second;
...
uint32_t MGetCurrentYearTimeCount(int date, int time, int mode) {
    uint32_t retvalue = 0;
    uint32_t year;  // 年
    uint32_t month; // 月
    uint32_t day;   // 日
...
uint32_t MGet2kDays(int mode) {
    uint32_t days;
    if (mode == 0)
        days = SystemStatus.days;
    else {
        TIME_CLOCK TimeClock;
        uint32_t   date, year, month;
...
uint32_t MGetPulseWidthNumber() {
    return SystemStatus.prepare[0];
}
void MSetPulseWidthInput(uint32_t pulsewidth, int mode /*mode = 0\1\2*/) {
    if (mode == C_SETMODE_SAVE || mode == C_SETMODE_SETSAVE) {
...
/// 设置参数自定义
uint32_t MGetDefPara(uint32_t iCh) {
    return (SystemStatus.prepare[1] >> iCh) & 0x01;
}
void MSetDefPara(uint32_t value, uint32_t iCh) {
    uint32_t iBit           = 0x01;
    uint32_t iTemp          = ~(iBit << iCh) & SystemStatus.prepare[1];
    SystemStatus.prepare[1] = iTemp + (value << iCh);
}

void MSet11345CurveNum(uint32_t iOn) {
    ChannelStatus.prepare[0] = iOn;
}

uint32_t MGet11345CurveNum() {
...
#else
        if (dx > 0)
            SetTransmitterVoltage(HIGH_PULSE);
        else
            SetTransmitterVoltage(LOW_PULSE);
#endif
    }
}
uint32_t MGetProbeMode(void) /*返回值为当前探头模式1\2\3\4\5=直\斜\双晶\穿透\双晶斜*/
{
    uint32_t Ax = ChannelStatus.sys;
...
uint32_t MGetFunctionMode(uint32_t type)
...
{
    uint32_t ax = func % 2; // 偶数为关，奇数为开
    uint32_t bx = 0xfffffffe;
    int      i;
...
void MSetScaleMode(int scale, int mode /*mode = 0*/) {
    ...
    cx = 24; // 移位
    dx = (scale % 3) & 0x03;
    if (MGetAngle(0) == 0 && dx == 1)
        dx++; // 直探头无水平标度
    if (MGetAngle(0) == 90 && dx == 2)
        dx--; // 直探头无水平标度
    ax = dx << cx;
    bx = 0x3;
...
int MGetParaStep(uint32_t paraindex) // 得到待调节参量步长
{
    int step = 0;
    //	uint32_t bx;
    switch (paraindex) {
...
//	uint32_t status;		//d31-d24仪器性能测试天数，d23d22测试项目：零点声速/K值/DAC\AVG,D21-d16探头测试天数(零点、K值、DAC)，
// d15－d12灵敏度测试天数
...
void MGetGatePeakBuffer() {
    ...
    if (Mabsi(curtime - pretime) > 10) // 最短10ms才读一次
    {
        ...
        if (AdjustTime.time > 100)
            AdjustTime.amp_total = AdjustTime.time = 0;
        if (AdjustTime.time2 > 100)
            AdjustTime.amp_total2 = AdjustTime.time2 = 0;
        // MSetAcquisitionEnable(1,C_SETMODE_SET);
    }
}
...
int MGetGateParaMax(int gatetype) {
    ...
    if (gateamp > GateMax.Amp) {
        ...
        for (; start < end; start++) {
            if (*(sampbuffer + start) > *(sampbuffer + pos))
                pos = start;
        }                                                            // 查找门内最高波
    ...
    // if(MGetAcquisitionEnable() == 1 )
    {
        ...
        if (gatetype == 0) {
            ...
            if (Mabsi(gateamp) < 64 * 2 && Mabsi(gateamp) > 1) {
                ...
                gain /= 128;
                if (gateamp > 0)
                    gateamp += AmpMaxCom_Table[gain * 8 + gateamp / 16];
                else
                    gateamp -= AmpMaxCom_Table[gain * 8 + Mabsi(gateamp) / 16];
            }
#endif
            int amp1 = gateamp;
...
int MGetAmpPos1(int gatetype, int mode) /*得到指定门内回波mode=0开始/1最高/2结束3/前端门相交/4后端门相交的点数，*/
{
    ...
    switch (mode) {
        ...
        case 4: // 整个波形后端与门相交的点
            ...
            break;
        case 5: // 整个波形一半的点
            if (*(sampbuffer + pos + 1) > *(sampbuffer + pos - 1))
                pos++;
            break;
        ...
        case 10: // 时间
            if (MGetEchoMode() != C_RF_WAVE) {
                if (gatetype == 0) {
                    ...
                    pos = GatePeak.Gate2PeakSampPoints;
            } else {
...
int MGetAmpTime(int gatetype) /*得到指定门（A or B)内回波传输时间，*/
{
...

    {
        ...
        if (MGetEchoMode() != C_RF_WAVE) {
            ...
            if (gatetype == 0)
                AmpTime = GatePeak.Gate1PeakSampPoints; //-2000 ;
            else if (gatetype == 1)
                AmpTime = GatePeak.Gate2PeakSampPoints;
        } else {
...
int MGetAmpDist(int gatetype) {
    if (gatetype == -1)
        return (GatePeak.Gate1PeakSampPoints * MGetSpeed() / (C_SAMPLE_FREQ));
    else if (gatetype == -2)
        return (GatePeak.Gate2PeakSampPoints * MGetSpeed() / (C_SAMPLE_FREQ));
    else if (gatetype >= 0 && gatetype < 2)
        return MGetAmpTime(gatetype) * MGetSpeed() / (C_SAMPLE_FREQ);
    else
        return -1;
}
int MGetAmpDist1(int gatetype) {
    double dist;
    if (gatetype == -1)
        dist = (MGetAmpTime(0) * MGetSpeed() / (double)(C_SAMPLE_FREQ));
    else if (gatetype == -2)
        dist = (MGetAmpTime(1) * MGetSpeed() / (double)(C_SAMPLE_FREQ));
    else if (gatetype >= 0 && gatetype < 2)
        dist = MGetAmpTime(gatetype) * MGetSpeed() / (double)(C_SAMPLE_FREQ);
    else
        return -1;
    dist += MGetDelay(3);
...
uint32_t MGetInputMode(void) /*返回值为当前输入法类型1\2\3\4\5=数字\大写\小写\拼音\区位*/
{
    uint32_t ax = SystemStatus.sys; /**/
    ax          = ax >> 26;
    ax          = ax & 7;
...
void MSetFill(uint32_t fill, int mode /*mode = 0\1\2*/) {
    fill %= 2;
    if (mode == C_SETMODE_SAVE || mode == C_SETMODE_SETSAVE) {
...
{
    uint32_t ax = SystemStatus.option; /**/
...
int MGetCraft() // 得到工艺号
{
    uint32_t ax = SystemStatus.option; /*5位*/
...
uint32_t MGetColorMode(void) // 是否
{
    uint32_t ax = SystemStatus.option; /**/
...
int SystemParaStore(void) /*系统参数存到保护区*/
{
    ...
    uint32_t addr = (uint32_t)&SystemStatus;
