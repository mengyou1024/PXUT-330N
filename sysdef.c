...
void SystemLog(void )
{
    ...
    TextOut(120+32, 340+24*2-6,  1, 8+14*MGetLanguage(),32, (u_char*)_MainInfo[MGetLanguage()][0], 4);
    if(MGetLanguage()==0)
        TextOut(120+32+5*24, 340+24*2-6,  1, 14,32, (u_char*)_MainInfo[MGetLanguage()][1], 4);
    TextOut(120+32+24,340+24*3,  1, 20,32, (u_char*)_MainInfo[MGetLanguage()][2], 4);
    ...
    while( GetElapsedTime() < elapsedtime && !bCOM2PC) /*5sec 后或有按键退出*/
    {
        ...
        if( keycode == 1 && SameKeyTime >= C_SAMEKEYTIME)
        {
            ...
            // }
			TextOut(xpos, ypos+row_height*0-C_ECHAR_VDOT*MGetLanguage(),  1, 19,24, (u_char*)_InnerInfo[MGetLanguage()][0], 4);
			TextOut(xpos, ypos+row_height*1-C_ECHAR_VDOT*MGetLanguage(),  1, 19,24, (u_char*)_InnerInfo[MGetLanguage()][1], 4);
			TextOut(xpos, ypos+row_height*2-C_ECHAR_VDOT*MGetLanguage(),  1, 19,24, (u_char*)_InnerInfo[MGetLanguage()][5], 4);
			TextOut(xpos, ypos+row_height*3-C_ECHAR_VDOT*MGetLanguage(),  1, 19,24, (u_char*)_InnerInfo[MGetLanguage()][6], 4);
			TextOut(xpos, ypos+row_height*4-C_ECHAR_VDOT*MGetLanguage(),  1, 19,24, (u_char*)_InnerInfo[MGetLanguage()][7], 4);
 ...
 */
                ...
                ypos = para_ypos + row_height *4;
                if(InnerInfo.Channel<20)InnerInfo.Channel=20;
...
//                    CopyMemoryToSystem((void*)offset,sizeof(INNER_INFO), PM_COPY_WRITE);
                ...
                case 5: //通道数
                    while(1)
                    {
                        Fixgain=0;
                        Input_Number(xpos,ypos,&Fixgain,3,&deci_len,1);
                        if(Fixgain>19 && Fixgain<51 )break;
...
//                            memset(&g_UDiskInfo,0,sizeof(UDISK_INFO));
        ...
        else if(keycode==C_KEYCOD_PLUS&&SameKeyTime >= C_SAMEKEYTIME)//2019.3.25 参数自动设置
        {
            ...
            while(1)
            {
                ...
                if(MGetDefPara(0)==0)  TextOut(iXPos+100,4+iHeight*0,1,6,24,"默认",4);
                else TextOut(iXPos+100,4+iHeight*0,1,6,24,"自定义",4);

                if(MGetDefPara(1)==0)  TextOut(iXPos+100,4+iHeight*1,1,6,24,"默认",4);
                else TextOut(iXPos+100,4+iHeight*1,1,6,24,"自定义",4);

                if(MGetDefPara(2)==0)  TextOut(iXPos+100,4+iHeight*2,1,6,24,"默认",4);
                else TextOut(iXPos+100,4+iHeight*2,1,6,24,"自定义",4);
                ...
                if(iKeycode==C_KEYCOD_RETURN||iKeycode==C_KEYCOD_CONFIRM) break;
        ...
        else if( keycode == C_KEYCOD_RETURN )
        {
            ...
            // }
			TextOut(xpos, ypos+row_height*0-(C_ECHAR_VDOT-4)*MGetLanguage(),  1, 19,row_height, (u_char*)_UserInfo[MGetLanguage()][0], 4);
			TextOut(xpos, ypos+row_height*1-(C_ECHAR_VDOT-4)*MGetLanguage(),  1, 19,row_height, (u_char*)_UserInfo[MGetLanguage()][1], 4);
			TextOut(xpos, ypos+row_height*2-(C_ECHAR_VDOT-4)*MGetLanguage(),  1, 19,row_height, (u_char*)_UserInfo[MGetLanguage()][5], 4);
