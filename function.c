...
int SearchPeak(uint8_t * sampbuff,uint32_t curr_pos,uint32_t direction)	//返回最高波所在水平点数，如为负数则为无最高波
{
    ...
    uint8_t preamp = *(sampbuff+curr_pos);	//前一波峰值
    uint8_t amp = *(sampbuff+curr_pos);	//当前波峰值

    uint16_t search_time = 0;	//寻找次数，不超过1
    short retvalue = 0;
...
int MSetCurveAmend(CURVE_AMEND *stCurveAmend)//参数正确可计算返回1，否则返回0；
{
    ...
    stCurveAmend->Beta = (uint16_t)( beta*10000+0.5);//半弦长对应的角度
    stCurveAmend->Half_Len_Bow = ( uint16_t)(half_len_bow*10+0.5);//半弦长
    stCurveAmend->Half_len_Arc = ( uint16_t)(half_len_arc*10+0.5);//半弦长对应的弧长
    /*
