...
int MGetKeyCode(uint32_t time)
{
    ...
    */
    ...
    uint8_t samekeytime;		//同一键值连续按键次数
    int offset;
...
void MDrawLine(uint32_t x1, uint32_t y1, uint32_t x2, uint32_t y2,uint32_t BGColor )   //BGColor,背景色
{
    int temp;
...
uint32_t DrawMultiEdge(int xpos,int ypos,uint32_t edge,float angle,int len)	/*画多边形*/
{
...
void DrawCursor(uint32_t xpos,uint32_t ypos,uint32_t mode)	/*画光标，mode=0英文1中文*/
{
...
int Write_UNumber(uint32_t xpos,uint32_t ypos,uint32_t number,uint32_t total_len,uint32_t deci_len)
{
    ...
    uint8_t AscChar;
    uint32_t int_number,deci_number,power_number; /*整数，小数，幂 */
...
void Write_Crystals(uint32_t xpos,uint32_t ypos,uint32_t crystal_l,uint32_t crystal_w)		/*写晶片尺寸*/
{
    int number;
...
int Input_Crystal(uint32_t xpos1,uint32_t ypos1,int *number_l,int *number_w)			//输入晶片尺寸
{
    ...
    uint32_t s_xPos =xpos ;		//初始水平位置
    uint32_t int_len ;			//代表当前输入的整数位
    uint32_t int_len1 =0;			//代表当前输入的整数位
    uint32_t deci_len1=0;	//代表当前输入的小数位
    uint32_t int_len2 =0;			//代表当前输入的整数位
    uint32_t deci_len2=0;	//代表当前输入的小数位
    uint32_t exitcode=0;
...
int Input_Float(uint32_t xpos,uint32_t ypos,float *number,uint32_t int_len,uint32_t deci_len,uint32_t sign)
{
    ...
    uint32_t total_len,temp_deci_len ; /*总长度*/
    uint32_t s_xPos = xpos;		/*初始水平位置*/
    int keycode;
...
{
    uint16_t xpos,ypos;
    uint8_t char_len,row_number,row_height;	/*字符数和字符行数*/
...
{
	uint32_t xpos,ypos;
	uint32_t char_len,row_number,row_height;	/*字符数和字符行数*/
	int keycode;
	int retvalue;
...
//	CMenuOut(xpos, ypos, _Message_B1 + index * char_len, char_len, row_number, row_height);
    ...
    else if(MGetLanguage()==0)
    {
        ...
        ///TextOut(xpos+2, ypos+2, 1, 20,row_height, (uint8_t*)"退出当前测试?Y/N", 4);
        int keycode1=0;
        while(1)
        {
...
//*
    ...
    uint32_t date = Bcd2Hex( (tm->date) & 0x3f );	// 日期  BCD码转换成十六进制
    /*/
    	uint32_t year =  tm->yearh  * 100 + tm->yearl ;
        uint32_t month =tm->month & 0x1f ;
    	uint32_t date = tm->date & 0x3f ;	// 日期  BCD码转换成十六进制
    //*/
    /* 2003-05-02 */
    if(year < 1000)year += 1000;
    Write_Number(xpos , ypos,year,4,0,0);
...
{
//*
    uint32_t hour = ( (tm >> 16) & 0x3f) % 24;// 提取有效位 并转换为十六进制
...
uint32_t InputDate(uint32_t xpos, uint32_t ypos, TIME_CLOCK *tm)
{
    ...
    uint32_t keycode = C_KEYCODMAX ;		/* 附值为不用的键值 */
...
int TextOut(int x,int y,int row,int len,int height, uint8_t* szText, int dots)
{
    ...
    uint8_t *pTemp;	//临时指针
    short i,j;
    uint16_t len_dots;//每行实际字符占用的点数
    uint8_t align;//每行的对齐方式
    uint8_t rh;
...
int GetStrLen( uint8_t * szText)//得到字符串的长度
{
    int len;
    for( len = 0; ; len++)
    {
