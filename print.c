...
const uint8_t Init_HP[]    = {18,
                              ...
                              0x1b, 42, 116, 49, 48, 48, 82}; // 以上设置成150 Dots Per inch,Ec * t150R
...
void HPTP(uint32_t xpos, uint32_t ypos, uint8_t PrnBuff[]) // 热敏打印机 针打垂直12针变为24针
{
    uint8_t Ax, Cx;
    int     i, j, k;
...
uint16_t Byte2Word(uint8_t Byte) // 一个字节变为一个字
{
...
{
    uint16_t PrintHoriBytes = 39; // 每行40个字节
...
uint32_t ScreenCopy2(int vdots, int hbytes) // 屏拷
{
    // 每行40个字节
    uint32_t xpos = 0, ypos = 0;
    int      i, j;
...
{
    uint16_t PrintHoriBytes = 64; // 每行40个字节
    uint32_t xpos = 0, ypos = 0;
    int      i;
    ...
    if (mode1 == 0) {
        ...
        if (mode2 >= 10) {
            ...
            if (mode2 == 10)
                ...
                pzText[5] = (int)(powf(10, (MGetLineGain(2) + 120) / 400.0) + 0.05) + 0x30;
            pzText[6] = ' ';
            pzText[7] = ' ';
        }
...
int ReportText2(int mode, int file_no) // 报告文本
{
    ...
    //	cpText = (uint16_t*)_ParaMenu_AB2 + ( (MGetProbeMode()-0)*4+1 );	//类型
...
int ReportText(int mode, int file_no) // 报告文本
{
    ...
    //	cpText = (uint16_t*)_ParaMenu_AB1 + (MGetScaleMode()-0) * 3 + 1;//声程标度
...
int ReportTextE(int mode, int file_no) // 报告文本
{
    ...
    if (MGetBGateMode() == 0)
        EMenuOut(xpos + 2, ypos + row_height + 8, _Report_B42, 10, 6, row_height);
    else
        EMenuOut(xpos + 2, ypos + row_height + 8, _Report_B43, 10, 6, row_height);
