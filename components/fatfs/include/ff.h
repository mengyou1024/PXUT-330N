...
FRESULT f_open (FIL*, const XCHAR*, uint8_t);			/* Open or create a file */
...
FRESULT f_getfree (const XCHAR*, uint32_t*, FATFS**);	/* Get number of free clusters on the drive */
...
FRESULT f_chmod (const XCHAR*, uint8_t, uint8_t);			/* Change attriburte of the file/dir */
...
/* Sync functions */
#if _FS_REENTRANT
bool ff_cre_syncobj(uint8_t, _SYNC_t*);
...
unsigned SdCard_GetCapacity(uint32_t *pFreeCluster, uint32_t *pLastCluster);

int Input_Gain(uint32_t xpos,uint32_t ypos,int *number,uint32_t int_len,int *deci_len,uint32_t sign,int *signFlag);
