        ...
        if ((MGetInputMode() == C_INPUT_PINYIN || MGetInputMode() == C_INPUT_QUWEI) && type > 0) {
            while (1) {
                ...
                if (MGetInputMode() == C_INPUT_PINYIN)
                    ...
                    pzText[0] = MGetQuwei(xpos, ypos, association);
                if (pzText[0] == 0)
                    break;
                else if (pzText[0] >= 32 && char_len < char_max) /*输入有效字符*/
                {
                    char_len++;
                    for (i = char_len; i > cursor_posi; i--)
            ...
            while (1) {
                ...
                {
                    if (insert_sign == 0) {
                        for (i = char_len + 1; i > cursor_posi; i--)
                            InputChar[i] = InputChar[i - 1];
                        char_len++;
                        cursor_posi++;
                    } else {
    ...
    do {
        ...
        if (input_degree == 0) {
            ...
            if (association > 255) {
                ...
                if (page_num == page_no + 1)
                    page_word_num = (_HFWORD[0]) % C_MAX_PAGE_WORD;
                else
                    page_word_num = C_MAX_PAGE_WORD;
                for (k = 0; k < page_word_num; k++) {
                    Write_Ascii(t_xpos, t_ypos, k + 48); /*写提示数字0-9*/
                    InvertBytes(t_xpos, t_ypos, 1);      /*反白显示数字*/
            ...
            for (wordindex_posi = 0; wordindex_posi < C_PINYIN_TOTAL; wordindex_posi++) {
                for (k = 0; k < input_degree; k++) {
                    if (WordIndex[wordindex_posi][k] != input[k])
                        break;
                }
                if (k >= input_degree)
                    break; // 全部比较过说明匹配成功则中断退出
            }
            if (input_degree != 0) {
                page_num = (pWordIndex[wordindex_posi][0] + C_MAX_PAGE_WORD - 1) / C_MAX_PAGE_WORD;
                /*当前拼音对应的汉字除以每页能显示的最多字数，得到当前拼音对应的汉字页数*/
                if (page_num == page_no + 1)
                    page_word_num = (pWordIndex[wordindex_posi][0]) % C_MAX_PAGE_WORD;
                else
                    page_word_num = C_MAX_PAGE_WORD;
                if (page_word_num == 0 && page_num != 0)
                    page_word_num = C_MAX_PAGE_WORD; /*当前拼音对应的汉字页中的字数，最大9*/
            } else {
                page_num      = 0;
                page_word_num = 0;
            }
        ...
        while (1) {
            if (GetElapsedTime() % 400 < 30)
                DrawCursor(xpos, ypos, 1); /*画光标，mode=0英文1中文*/
        ...
        if (keycode == C_KEYCOD_RETURN && i >= C_MAX_SAMEKEYTIME / 5) {
            ...
            if (input_degree == 0) /*有联想*/
            {
                if (association > 255)
                    ...
                    pzText[0] = CH(_HFWORD[keycode + 1 + page_no * C_MAX_PAGE_WORD]);
            } else /*无联想*/
        ...
        } else if (keycode == C_KEYCOD_CONFIRM) //  && i >= C_MAX_SAMEKEYTIME/5 )
        {
            ...
            if (input_degree > 0) {
                ...
                while (pinyin_posi > 0) {
                    ...
                    for (i = 1; num2char[pinyin_posi][i] != ' '; i++)
                        ; /*发现空格，输入结束*/
                    if (i < input_degree)
                        break; /**/
                }
...
/*得到ASCII字符*/
uint16_t MGetEChar(uint32_t xpos, uint32_t ypos, uint16_t* pzText, uint32_t mode) {
    ...
    do {
        if (pzText[0] >= 32)
            Write_Ascii(xpos, ypos, pzText[0]);
        ...
        while (1) {
            ...
            if (Prekeycode == C_KEYCODMAX)
                if (GetElapsedTime() % 400 < 5)
                    DrawCursor(xpos, ypos, 0); /*画光标，mode=0英文1中文*/
...
int MGetQuwei(uint32_t xpos, uint32_t ypos, uint16_t association) {
    ...
    do {
        ...
        if (association > 255 && input_degree == 0) {
            ...
            /*当前拼音对应的汉字除以每页能显示的最多字数，得到当前拼音对应的汉字页数*/
            if (page_num == page_no + 1)
                page_word_num = word_num % C_MAX_PAGE_WORD;
            else if (page_num != 0)
                page_word_num = C_MAX_PAGE_WORD;
            if (page_word_num == 0 && page_num != 0)
                page_word_num = C_MAX_PAGE_WORD; /*当前拼音对应的汉字页中的字数，最大9*/
